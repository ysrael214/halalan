...
echo "</table>";
	...
	function breakdown() {
		$p = 1;
		$query = $this->db->query("SELECT id, block FROM blocks ORDER BY block ASC");

echo "<table border=2><tr>";
echo "<th></th>";
	foreach ($query->result() as $row):
{
	echo "<th>$row->block</th>";
}
endforeach;
echo "<th>TOTAL</th></tr>";

echo "<tr><td><b>Chairperson</b></tr></td>";

$query2 = $this->db->query("SELECT id, alias, last_name FROM candidates WHERE position_id = $p ORDER BY party_id ASC");

foreach ($query2->result() as $row):
{
	echo "<tr><td>$row->alias $row->last_name</td>";
	$candi = $row->id;
	$sum = 0;
		foreach ($query->result() as $row):
		{
			$query3 = $this->db->query("SELECT COUNT(*) FROM voters v, votes s WHERE v.id = s.voter_id AND v.block_id = $row->id AND s.candidate_id = $candi");
			$result = $query3->row_array();
			$count = $result['COUNT(*)'];
			echo "<td>$count</td>";
			$sum = $sum + $count;
...
endforeach;
	echo "<tr><td>ABSTAIN</td>";
	$sum = 0;
		foreach ($query->result() as $row):
		{
			$query4 = $this->db->query("SELECT COUNT(*) FROM abstains a, voters v WHERE v.id = a.voter_id AND v.block_id = $row->id AND a.position_id = $p");
...
echo "<tr><td><b>Vice Chairperson</b></tr></td>";
...
echo "<tr><td><b>Councilors</b></tr></td>";
